...
export default function ManageTreks() {
  ...
  const handleDelete = async (trek) => {
    if (
      window.confirm(
        `Are you sure you want to permanently delete "${trek.title}"?`,
      )
    ) {
      try {
        setLoading(true);
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-[2.5rem] p-12 text-center border border-gray-100 shadow-sm">
            ...
            <p className="text-gray-500 font-bold max-w-sm mx-auto">
              {searchQuery
                ? `We couldn't find any treks matching "${searchQuery}".`
                : "You haven't added any treks to the catalog yet."}
            </p>
            {searchQuery && (
              <button
          ...
          <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-300">
              ...
              <div className="flex-1 space-y-3">
                ...
                <div className="flex flex-wrap items-center gap-4 text-sm text-gray-500">
                  ...
                  <div className="flex items-center gap-1.5 font-bold text-gray-800">
                    ₹{" "}
                    {(selectedTrek?.feeDetails?.totalFee ?? 0).toLocaleString()}
                  </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 p-4 bg-gray-50 rounded-xl border border-gray-100">
              ...
              <div>
                ...
                <p className="text-sm font-bold text-gray-800">
                  ₹ {selectedTrek?.feeDetails?.totalFee || 0}
                </p>
              </div>
            </div>
